...
// double crossProduct(TVector a,TVector b,TVector c)//向量 ac 在 ab 的方向 
// {
...
int grahamScan()
{
	...
	for(int i=2; i<points.size(); i++)
	{
		...
		while(xyd(convexHull[top].orientation(convexHull[top-1],points[i]), 0.0) && top >= 1)
			top--;
		// while( xyd( crossProduct(convexHull[top],convexHull[top-1],points[i]), 0.0 ) && top >= 1 )
		// 	top--;
...
int main(){
	...
	ll n;
 	...
 	// scanf ("%lf %lf", &b.x, &b.y);
 	// printf("%d\n",e.orientation(a,b) );
 	// long double teste =  (0.1)+(0.2);
 	// long double teste2 = (1.4)*(1.4)*(1.4) ;
 	// printf("%.18LF %.18LF\n",teste,teste2 );
 	// printf("%LF %LF\n",teste,teste2 );
